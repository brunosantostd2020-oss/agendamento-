  ...
  try {
    ...
    const resumos = await Promise.all(funcs.map(async f => {
      ...
      return {
        ...
        atendimentos: ags.length, total_bruto: bruto,
        comissao_pct: f.comissao_pct, comissao_valor: comissao,
        salario_fixo: f.salario_fixo, total_a_pagar,
      };
    }));
    ...
    res.json({
      ...
      sem_funcionario: { qtd: parseInt(semFuncRow.qtd), total: parseFloat(semFuncRow.total) },
  ...
  try {
    ...
    await pool.query(
      ...
      [id, req.session.userId, nome.trim(), cargo||'', telefone||'', email||'',
       parseFloat(salario_fixo)||0, parseFloat(comissao_pct)||0, cor||'#00d084', agora]
    );
...
// ── VINCULAR AGENDAMENTO (estático, ANTES de /:id) ────────────
...
router.patch('/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const r = await pool.query(
      ...
      [nome.trim(), cargo||'', telefone||'', email||'',
       parseFloat(salario_fixo)||0, parseFloat(comissao_pct)||0,
       cor||'#00d084', ativo !== false, agora,
       req.params.id, req.session.userId]
    );
    if (r.rowCount === 0) return res.status(404).json({ erro: 'Funcionário não encontrado.' });
    res.json({ sucesso: true });
  } catch(e) { res.status(500).json({ erro: e.message }); }
});
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    await pool.query(
      'DELETE FROM funcionarios WHERE id=$1 AND negocio_id=$2',
      [req.params.id, req.session.userId]
    );
    res.json({ sucesso: true });
  } catch(e) { res.status(500).json({ erro: e.message }); }
